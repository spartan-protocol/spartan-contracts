...
constructor (address _base) {
 ...
 function HandlerAddr() internal view returns(iHandler) {
    ...
    function calcLiquidityUnits(uint256 token1Input, uint256 token1Depth, uint256 token2Input, uint256 token2Depth, uint256 totalSupply) external pure returns (uint256 liquidityUnits) {
        if (totalSupply == 0) {
            return token1Input; // If pool is empty; use token1Input as initial units
        } else {
            // units = ((P (t B + T b))/(2 T B)) * slipAdjustment
            // P * (part1 + part2) / (part3) * slipAdjustment
            uint256 slipAdjustment = getSlipAdjustment(token1Input, token1Depth, token2Input, token2Depth);
    ...
    function getSlipAdjustment(uint256 token1Input, uint256 token1Depth, uint256 token2Input, uint256 token2Depth) public pure returns (uint256 slipAdjustment) {
        ...
        uint256 denominator = token2Depth.mul(2).add(token2Input).mul(token1Depth.add(token2Depth));
        if (denominator == 0) {
            return ONE;
        }
